...
model.add(tf.keras.layers.Conv2D(32, kernel_size=3, strides=1, padding='valid', input_shape=(4, 45, 45),
...
model.add(tf.keras.layers.Dense(256, kernel_initializer=tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.01, seed=None), bias_initializer='zeros'))
model.add(tf.keras.layers.BatchNormalization())
model.add(tf.keras.layers.Activation('relu'))
...
'''Train the network'''
model.fit(X, Y, epochs=50, batch_size=100, validation_data=(X_vali, Y_vali), verbose=1, shuffle=True)

model.save("C:/Users/yifan/Google Drive/PythonSync/wasabi-detection-python/Models/Regression/saved_model.model")
writer_fid = open("C:/Users/yifan/Google Drive/PythonSync/wasabi-detection-python/Models/Regression/saved_image_norm.model", "wb")
