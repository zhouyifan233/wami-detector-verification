...
def run(d_out='savefig', frames_to_attack=[], ref_track=None):
  ...
  for i in range(20):
      ...
      if i > 0:
          ...
          if isinstance(refinementID, np.int64) and (i in frames_to_attack):
            #######
      ...
      if ref_track is None or i==0:
        draw_error_ellipse2d(roi_image, (kf.mu_t[0]-minx, kf.mu_t[1]-miny), kf.sigma_t)
        #f = open('path_attacked_255.txt','a')
        f = open('{0}/track_res.txt'.format(d_out),'a')
        f.write('{0} {1} {2}\n'.format(i, kf.mu_t[0]-minx, kf.mu_t[1]-miny))
        #f.write('{0} {1} {2} {3}\n\n'.format(i, kf.mu_t[0]-minx, kf.mu_t[1]-miny, kf.sigma_t))
        f.close()
      else:
        draw_error_ellipse2d(roi_image, (ref_track[i].x, ref_track[i].y), ref_track[i].delta)
        draw_error_ellipse2d(roi_image, (kf.mu_t[0]-minx, kf.mu_t[1]-miny), kf.sigma_t, color='nk')
        f = open('{0}/track_res.txt'.format(d_out),'a')
        f.write('{0} {1} {2}\n'.format(i, kf.mu_t[0]-minx, kf.mu_t[1]-miny))
        f.close()
      ...
      cv2.imwrite(d_out + "/%05d.png"%i, roi_image)
      """
      plt.figure()
  ...
  if not (ref_track is None):
    f = open('{0}/track_res.txt'.format(d_out),'a')
    f.write('{0} {1}\n'.format(diff_mean(ref_track, detected_track), diff_max(ref_track, detected_track)))
    f.close()
...
if __name__ == '__main__':
  ...
  #for i in range(4,10):
  #  run('output-triple-frame-attack-i{0}'.format(i), [i, i+1, i+3], ref_track)

  ## attack all frames
  #frames=[]
  #for i in range(3, 20):
  #  frames.append(i)
  #run('output-n-n-attack', frames, ref_track)

  ## m=1 k=2
  #frames=[]
  #for i in range(3, 20):
  #  if i%2==1: frames.append(i)
  #  run('output-1-2-attack', frames, ref_track)

  ## m=1 k=3
  #frames=[]
  #for i in range(3, 20):
  #  if i%3==1: frames.append(i)
  #  run('output-1-3-attack', frames, ref_track)
